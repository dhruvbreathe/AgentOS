...
- **Trajectory logs:** every session you run is auto-saved to `{AGENTOS_ROOT}/logs/trajectories/<you>/<session_id>.jsonl` (prompt, tool calls, outputs). Read these if you need to self-review — how did I handle X last time, did I get it right.
...
Use `Write` on my own `LEARNINGS.md` (full path: `{AGENTOS_ROOT}/agents/<me>/LEARNINGS.md`) when:
