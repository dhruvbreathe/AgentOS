...
```bash
python /Users/celainc/Developers/ClaudeAgentSDK/discord-relay/scheduler/install.py
```
...
```bash
python /Users/celainc/Developers/ClaudeAgentSDK/discord-relay/scheduler/install.py --apply
```

This writes plists to `~/Library/LaunchAgents/com.discord-relay.<agent>-<task>.plist` and loads them with `launchctl bootstrap gui/$UID`. No hooks block it, no TCC prompt.
...
```bash
python /Users/celainc/Developers/ClaudeAgentSDK/discord-relay/scheduler/install.py --list
```

Shows every currently-loaded `com.discord-relay.*` job. Verify my new one is there and tell the operator it's live.
...
2. Deletes everything between `# --- discord-relay (managed) ---` and `# --- /discord-relay ---`
...
- **Every task writes a log line** at `discord-relay/logs/<agent>-<task>.log`. Nothing runs silently.
