"""launchd-based scheduler for discord-relay agent tasks.
...
plist, writes it to `~/Library/LaunchAgents/com.discord-relay.<agent>-<task>.plist`,
...
LABEL_PREFIX = "com.discord-relay"
UID = os.getuid()
...
def _cron_active_tasks() -> set[tuple[str, str]]:
    ...
    managed discord-relay block. Used to avoid installing a launchd job whose
    ...
    for line in out.splitlines():
        if "# --- discord-relay (managed) ---" in line:
            in_block = True
            continue
        if "# --- /discord-relay ---" in line:
            in_block = False
            continue
        if not in_block:
            continue
    ...
    if cron_active and not force:
        ...
        if skipped:
            print(
                ...
                "`# --- discord-relay (managed) ---` markers) then re-run "
